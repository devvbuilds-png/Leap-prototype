# CLAUDE.md — Leap: "Next Leap" Career Outcome Simulator
...
- **LLM:** OpenRouter API (model: anthropic/claude-3.5-sonnet), called via Next.js API routes (server-side)
- **Deployment:** Vercel
...
Status: ✅ Done
Changes/decisions:
- API route: POST /api/recommend → OpenRouter (anthropic/claude-3.5-sonnet), returns JSON array of 3 PathOption objects.
